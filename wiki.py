...
if os.path.exists(wiki_dir):
    ...
    sh = 'mkdir wiki && cd wiki && git init &&  git remote add origin\
        git@112.74.93.140:data-collector/email_station.wiki.git'
...
api = {
    'API-Schedule': [
        api.schedule.api_get_schedules
    ],
}

wiki_model = {
    'Model-Schedule': schedule.Schedule,
}

# Generate wiki pages for apis.
for page_name, funcs_list in api.items():
